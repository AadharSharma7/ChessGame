...
class Piece(pygame.sprite.Sprite):
    ...
    def check_board_position_free(self, pieces):
        for p in pieces:
            for i in range(8):
                for j in range(8):
                    if p.x in range(chess_board[i][j].x, chess_board[i][j].x + 80) and p.y in range(chess_board[i][j].y, chess_board[i][j].y + 80):
                        # print("position - " + str(chess_board[i][j]) + ", " + str(i) + ", " + str(j) + " is NOT free")
                        # print("it has a " + p.__str__())
                        self.not_free_x.append(p.x)
                        self.not_free_y.append(p.y)
                    else:
                        self.free_x.append(p.x)
                        self.free_y.append(p.y)
...
class Knight(Piece):
    ...
    def move(self, new_x, new_y):
        if self.click:
            self.move_child(new_x, new_y, self.circle_positiveX1)
            self.move_child(new_x, new_y, self.circle_positiveX2)
            self.move_child(new_x, new_y, self.circle_negativeX1)
            self.move_child(new_x, new_y, self.circle_negativeX2)
            self.move_child(new_x, new_y, self.circle_positiveY1)
            self.move_child(new_x, new_y, self.circle_positiveY2)
            self.move_child(new_x, new_y, self.circle_negativeY1)
            self.move_child(new_x, new_y, self.circle_negativeY2)
        self.kill_circles()
        self.click = False
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if event.button == 1:
                for p in all_pieces:
                    ...
                    if p.click:
                        # p.check_board_position_free(all_pieces)
